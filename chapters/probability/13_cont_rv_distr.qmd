...
Nel @sec-prob-discr-distr abbiamo esplorato le distribuzioni discrete come strumenti per rappresentare credenze su fenomeni con esiti numerabili. In questo capitolo estendiamo il framework alle *distribuzioni continue*, che permettono di esprimere credenze su quantità che variano su scale numeriche continue, come tempi, distanze, proporzioni, tassi e punteggi psicometrici. In questo contesto, la probabilità non viene assegnata a valori singoli, ma a intervalli tramite funzioni di densità.

Le distribuzioni continue svolgono un ruolo complementare a quello delle distribuzioni discrete nel framework bayesiano. Come **modelli di verosimiglianza per dati continui**, diverse famiglie di distribuzioni catturano differenti processi generativi: la normale per le misurazioni affette da errori simmetrici, l’esponenziale per i tempi di attesa e la gamma per le variabili positive asimmetriche. Le stesse o altre distribuzioni sono inoltre utilizzate per esprimere e aggiornare le nostre credenze come **prior e posterior su parametri continui**, ad esempio la beta per probabilità $p \in (0,1)$, la gamma per tassi $\lambda > 0$, la normale per le medie $\mu \in \mathbb{R}$, e la Cauchy e la $t$ di Student come prior robusti a code pesanti.

Una caratteristica elegante dell’inferenza bayesiana è la **dualità discreta–continua** espressa dalle famiglie coniugate: la distribuzione binomiale si accompagna naturalmente alla beta e la Poisson alla gamma. Questa simmetria, introdotta nel capitolo precedente, viene qui approfondita attraverso l’esplorazione delle distribuzioni continue che fungono da prior naturali per i parametri dei modelli discreti.

### Struttura del capitolo {.unnumbered .unlisted}

L’esplorazione seguirà una progressione che partirà da distribuzioni concettualmente semplici per arrivare a quelle con ruoli più specifici nell’inferenza bayesiana. Inizieremo con la distribuzione **uniforme**, che rappresenta una forma di ignoranza locale su un intervallo finito. Passeremo poi alla distribuzione **esponenziale**, fondamentale per modellare tempi di attesa, e alla distribuzione **normale**, centrale per il Teorema del Limite Centrale. Esamineremo poi la distribuzione **chi-quadrato** e la distribuzione **$t$ di Student**, che derivano dalla distribuzione normale e trovano applicazione nell’inferenza e nella modellazione robusta. Concluderemo con le distribuzioni di maggiore rilevanza bayesiana: la **beta** come prior per le proporzioni, la **gamma** come prior per i tassi e le varianze e la **Cauchy** come prior robusto a code pesanti.
...
::: {.callout-cautioncollapse="true" title="Preparazione del Notebook"}
...
La distribuzione uniforme continua rappresenta uno stato di **ignoranza locale** all’interno di un intervallo definito: in assenza di informazioni che favoriscano alcuni valori rispetto ad altri, tutti i valori dell’intervallo sono considerati ugualmente plausibili.

::: {#def-uniform-cont}
Una variabile casuale $X$ segue una distribuzione uniforme sull’intervallo $[a, b]$ se la sua densità di probabilità è costante:
...
**Proprietà**: $\mathbb{E}(X) = \frac{a+b}{2}$,
$\mathbb{V}(X) = \frac{(b-a)^2}{12}$.
:::

**Interpretazione epistemica**
La distribuzione uniforme codifica il *principio di indifferenza*: quando non abbiamo ragioni per preferire un valore rispetto a un altro all’interno dell’intervallo, assegniamo la stessa credenza a tutti i sottointervalli di uguale ampiezza. In questo senso, la distribuzione uniforme continua è l’analogo della distribuzione uniforme discreta.

### Ruolo nell’inferenza bayesiana

La distribuzione uniforme è spesso impiegata come **prior “non informativa”** quando si desidera rappresentare l’assenza di conoscenze pregresse su un parametro vincolato a un intervallo finito. Un esempio classico è la distribuzione $\text{Uniforme}(0, 1)$, che è equivalente alla distribuzione $\text{Beta}(1, 1)$ e rappresenta l’ignoranza iniziale riguardo a una proporzione.

**Limiti come prior**
Nonostante la sua semplicità, la distribuzione uniforme può risultare informativa in modo non intenzionale. In particolare:

* assegna probabilità zero ai valori esterni all’intervallo,
* dipende dalla scelta della scala e della parametrizzazione.

Per questo motivo, in molti contesti bayesiani si preferiscono prior con code pesanti, come la Cauchy o la $t$ di Student, che non escludono completamente valori estremi.

### Calcolo delle probabilità

Per una variabile casuale $X \sim \text{Uniforme}(a, b)$, la probabilità che $X$ assuma valori in un sottointervallo $[c, d] \subseteq [a, b]$ è proporzionale alla lunghezza del sottointervallo:
...
**Interpretazione epistemica**
Il parametro $\lambda$ rappresenta il tasso atteso di occorrenza degli eventi per unità di tempo. Valori elevati di $\lambda$ corrispondono a eventi frequenti e tempi di attesa brevi, mentre valori piccoli indicano eventi rari e tempi di attesa lunghi. Il valore $\frac{1}{\lambda}$ rappresenta il tempo medio di attesa.

### Proprietà di assenza di memoria

La caratteristica distintiva della distribuzione esponenziale è la *proprietà di assenza di memoria*: la probabilità che un evento si verifichi in un intervallo futuro è indipendente dal tempo già trascorso. Formalmente:

$$
P(X > s + t \mid X > s) = P(X > t).
$$

Questa proprietà riflette l’idea che il processo non “accumuli” né “consumi” probabilità con il passare del tempo: ogni istante è un nuovo inizio. L’esponenziale è l’unica distribuzione continua non degenere a possedere questa proprietà.

### Ruolo nell’inferenza bayesiana

La distribuzione esponenziale costituisce un caso particolare della distribuzione gamma ed è formalmente equivalente a una $\text{Gamma}(1, \lambda)$. Nel contesto dell’inferenza bayesiana essa svolge un duplice ruolo.

Come **funzione di verosimiglianza**, viene utilizzata per modellare variabili temporali continue, come tempi di attesa, durate di eventi o intervalli tra occorrenze successive. Per quanto riguarda l’aggiornamento bayesiano, la distribuzione **gamma** rappresenta la prior coniugata per il parametro di tasso $\lambda$, consentendo di ottenere una distribuzione a posteriori in forma chiusa e appartenente alla stessa famiglia.
...
- **Punteggi standardizzati**: QI (μ=100, σ=15), SAT, z-scores
- **Errori di misura simmetrici**: tempi di reazione (spesso dopo log-trasformazione)
- **Medie di campioni grandi**: per il Teorema del Limite Centrale
- **Prior su medie μ**: quando il parametro è non vincolato

**Esempio pratico:** Punteggi di depressione BDI in popolazione generale seguono approssimativamente N(μ=10, σ=8).

**Prior bayesiani tipici:**

- N(0, 10²): prior vaga su effetti standardizzati
- N(μ₀, σ₀²): prior informativo centrato su μ₀

**Attenzione:** Molte variabili psicologiche NON sono normali (es. tempi di reazione → log-normale, conteggi → Poisson).
:::
