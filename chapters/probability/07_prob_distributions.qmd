...
Nella statistica bayesiana, l'incertezza non è solo un limite da ridurre, ma una componente fondamentale da quantificare e gestire. Questo approccio si basa su una ridefinizione radicale del concetto di parametro: non più una quantità fissa da scoprire, ma una variabile casuale che riflette il nostro stato di conoscenza imperfetta.

Ad esempio, quando si stima un parametro come la media $\theta$ di una popolazione, l'approccio bayesiano assegna a $\theta$ una distribuzione di probabilità completa. Questa distribuzione non descrive una variabilità intrinseca del parametro (che potrebbe essere un valore fisso), ma esprime quantitativamente la plausibilità soggettiva di ciascun suo possibile valore sulla base dei dati osservati e delle conoscenze pregresse.

In questo contesto, la funzione di densità di probabilità (PDF) assume un significato completamente diverso. Non rappresenta più una frequenza relativa osservabile in esperimenti ripetuti, ma una sorta di "mappa" della nostra fiducia. Essa codifica il grado di credenza che il valore vero di un parametro o di una quantità incerta si trovi in un determinato intervallo. È come se "spalmassimo" la nostra incertezza su tutti i valori possibili: la forma della distribuzione, con le sue sfumature lungo l'asse $x$, visualizza la distribuzione delle nostre credenze.

Un'analogia efficace è quella della densità di materia in fisica. La funzione di densità $\rho(x)$ non specifica la posizione esatta di ogni singolo atomo, ma descrive come la massa è distribuita nello spazio. Allo stesso modo, una PDF bayesiana non descrive dove "cadrà" un dato in ipotetiche ripetizioni, ma come la nostra "massa di incertezza" è distribuita tra i diversi valori possibili.

Per rendere l'idea con un esempio concreto, immagina di dover stimare l'altezza media di una popolazione. Un bayesiano non pensa a infinite misurazioni ripetute, ma considera il valore medio stesso come un'incognita. La PDF bayesiana che ne risulta rappresenta, in ogni punto, quanto fortemente crediamo che quel valore specifico sia la media vera, avendo a disposizione un campione di dati e un'idea preliminare.

### Confronto

Le differenze tra l'approccio bayesiano e quello frequentista emergono chiaramente proprio nell'interpretazione della PDF.

L'interpretazione *frequentista* mantiene un focus sulla distribuzione dei dati. In questa prospettiva, la PDF descrive come i valori di una variabile $x$ si distribuirebbero se fosse possibile ripetere il processo di campionamento o l'esperimento un numero infinito di volte, mantenendo condizioni identiche. L'oggetto dell'analisi è il comportamento a lungo termine dei dati. Un esempio emblematico è l'istogramma che si otterrebbe da una serie molto lunga di esperimenti, che approssimerebbe la PDF teorica.

Al contrario, l'interpretazione *bayesiana* sposta l'attenzione sulla distribuzione della nostra incertezza o credenza. In questo caso, la PDF non ha bisogno del concetto di ripetizione infinita; essa riflette direttamente la plausibilità che attribuiamo a ciascun valore di $x$ (o di un parametro), sulla base di tutte le informazioni disponibili in quel momento. L'oggetto dell'analisi è lo stato della nostra conoscenza. L'esempio più rappresentativo è la distribuzione a posteriori di un parametro che sintetizza in modo coerente quanto imparato dai dati osservati con quanto si sapeva prima (l'informazione a priori).
...
In sintesi, questa distinzione tra interpretazioni non è solo una questione di semantica, ma ha implicazioni pratiche nella formulazione di modelli statistici e nell'interpretazione dei risultati. Mentre l'approccio frequentista è spesso utilizzato quando è possibile pensare concettualmente a un numero infinito di ripetizioni dell'esperimento, l'approccio bayesiano è particolarmente utile quando si desidera esprimere e aggiornare il proprio livello di incertezza riguardo a una quantità, basandosi sia su dati che su conoscenze pregresse.

## Riflessioni conclusive {.unnumbered .unlisted} 

La funzione di densità di probabilità (PDF) costituisce il fondamento per la descrizione delle variabili casuali continue e consente di associare le probabilità agli intervalli tramite il calcolo dell'area sottesa alla curva. In questo contesto, la probabilità di osservare un valore esatto è pari a zero, non perché l'evento è impossibile, ma perché in un insieme continuo ogni singolo punto contribuisce con un'area infinitesimale.

Il paradosso apparente secondo cui la somma di infiniti contributi nulli porta a una probabilità totale positiva si risolve grazie alla teoria dell'integrazione. Integrando i contributi infinitesimali lungo un intervallo, si ottiene una quantità finita che rappresenta la probabilità complessiva dell'evento. Un'interpretazione alternativa, fornita dalla teoria degli infinitesimi di Abraham Robinson, consente di attribuire a tali eventi probabilità infinitesimali, distinguendo diverse "grandezze" e chiarendo ulteriormente il processo di aggregazione verso il valore unitario.

Nel campo della *data science*, le distribuzioni di probabilità, formalmente rappresentate da $p(x)$, sono strumenti indispensabili per modellare la variabilità osservabile in una popolazione. Tali distribuzioni non mirano a riprodurre ogni aspetto della realtà in maniera dettagliata, ma offrono un modello semplificato che consente di generalizzare i dati osservati e di formulare previsioni rigorose sui fenomeni futuri. In altre parole, $p(x)$ non rappresenta la popolazione nel suo complesso, ma un'astrazione matematica che cattura l'incertezza e la variabilità del fenomeno studiato.
