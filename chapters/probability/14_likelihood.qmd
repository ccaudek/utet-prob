...
Nei capitoli precedenti abbiamo utilizzato le distribuzioni di probabilità come strumenti per descrivere il comportamento delle variabili casuali. Sotto l'ipotesi di un modello probabilistico con parametri noti, siamo stati in grado di calcolare la probabilità associata a determinati valori osservati o a specifiche configurazioni dei dati.

Nella pratica della ricerca empirica, tuttavia, il problema che ci si trova ad affrontare è spesso di natura inversa. Ad esempio, uno psicologo clinico osserva che 18 pazienti su 25 hanno risposto positivamente a un nuovo trattamento. L'interrogativo rilevante non è "qual è la probabilità di osservare 18 successi?", domanda che presupporrebbe un tasso di risposta noto, ma piuttosto "quanto è efficace questo trattamento?", ovvero: quali valori del parametro che esprime il tasso di risposta sono maggiormente compatibili con i dati osservati? In questa prospettiva, i dati costituiscono il punto di partenza e l'obiettivo è determinare quali valori parametrici siano più plausibili alla luce dell'evidenza empirica disponibile. Questo cambio di prospettiva, dal calcolo della probabilità dei dati condizionatamente al modello alla valutazione della plausibilità dei parametri condizionatamente ai dati osservati, rappresenta il nucleo concettuale dell'inferenza statistica.

È proprio per rispondere a questo problema inverso che viene introdotta la **funzione di verosimiglianza**. Essa non assegna una distribuzione di probabilità ai parametri, ma quantifica, per ciascun valore parametrico, il grado di plausibilità dei dati osservati, a condizione che la struttura del modello probabilistico sia correttamente specificata. In altri termini, la verosimiglianza definisce una mappa della plausibilità parametrica che consente di confrontare diversi valori dei parametri in base alla loro capacità di spiegare l'evidenza empirica.

In questo capitolo, la funzione di verosimiglianza viene presentata come concetto fondamentale dell'inferenza bayesiana. Nel capitolo successivo, ne esamineremo le principali proprietà matematiche e mostreremo come, una volta combinata con le distribuzioni a priori, conduca naturalmente e formalmente alla determinazione della distribuzione a posteriori.
...
Per analizzare questa funzione in modo più semplice, consideriamo la
\textbf{log-verosimiglianza}:

$$
\begin{aligned}
\ell(\mu) &= -\frac{n}{2}\log(2\pi\sigma^2) \\
&\quad -\frac{1}{2\sigma^2}\sum_{i=1}^{n} (y_i-\mu)^2.
\end{aligned}
$$
