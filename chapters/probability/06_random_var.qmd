...
Finora abbiamo lavorato con probabilità associate a *eventi discreti*, come vincere al gioco di Monty Hall o ottenere una diagnosi medica accurata. Questi esempi ci hanno permesso di introdurre l'idea fondamentale secondo cui una probabilità rappresenta un *grado di incertezza*: assegniamo il valore 0 a ciò che riteniamo impossibile, il valore 1 a ciò che consideriamo certo, e valori intermedi quando non siamo completamente sicuri.

Tuttavia, molti problemi reali richiedono una descrizione dell'incertezza più articolata. In psicologia, ad esempio, non ci interessa solo se un paziente possa migliorare, ma di quanto ci aspettiamo che migliori; non solo se una decisione sarà presa correttamente, ma con che frequenza nel lungo periodo; non solo se un comportamento si verificherà, ma quale valore assumerà una variabile fisiologica o cognitiva quando il comportamento si manifesta.

In queste situazioni, non vogliamo misurare la probabilità di un singolo evento, ma descrivere l'*intera distribuzione dei possibili risultati numerici* prodotti da un processo aleatorio.

Per esempio:

* in uno scenario tipo Monty Hall ripetuto più volte, potremmo voler stimare il *numero atteso di tentativi* necessari per ottenere una vittoria;
* in un contesto clinico, potremmo voler modellare la *durata prevista* di un trattamento;
* giocando con un dado sbilanciato per molte ore, potremmo voler valutare la *perdita economica media* sul lungo periodo.

Queste domande vanno oltre il “succede / non succede” e richiedono un linguaggio quantitativo più potente.

## Il ruolo delle variabili casuali

Per affrontare questi problemi introduciamo il concetto centrale di *variabile casuale*.
Una variabile casuale è una *funzione* che assegna un numero a ogni possibile esito di un esperimento aleatorio.

Si tratta di uno strumento fondamentale che permette di tradurre gli esiti qualitativi (per esempio "piove"/"non piove", "successo"/"fallimento", "tono emotivo positivo"/"tono emotivo negativo") in quantità numeriche analizzabili.

Da questo punto di vista:

* la mappatura dagli esiti ai numeri è *deterministica* (una funzione è sempre una funzione);
* la *casualità* proviene dall’incertezza sull’esito dell’esperimento, non dal fatto che la funzione cambi;
* la scelta dei numeri è spesso *arbitraria*, a condizione che sia mantenuta coerentemente durante l’analisi.

Una volta compiuto questo passo concettuale, è possibile applicare gli strumenti dell’analisi matematica per studiare il comportamento della variabile, calcolare i valori attesi, le varianze, le funzioni di ripartizione, le densità, le trasformazioni, le approssimazioni, e così via.

### Variabili casuali discrete e continue

Le variabili casuali si presentano principalmente in due forme:

* *discrete*, quando possono assumere un insieme *numerabile* di valori (come la categoria del tempo atmosferico domani, il numero di errori in un test, o il numero di tentativi fino al successo).
* *continue*, quando possono assumere *qualsiasi valore* in un intervallo reale (come il tempo di reazione, la temperatura corporea, o la latenza di una risposta neurale).

Ciascuna di queste tipologie richiede formulazioni matematiche specifiche per descrivere la distribuzione dei valori possibili e le loro proprietà.

### Panoramica del capitolo {.unnumbered .unlisted}

In questo capitolo faremo un passo decisivo che ci permetterà di passare dalle probabilità degli eventi alle probabilità dei valori numerici generati da processi aleatori, entrando così nel mondo delle distribuzioni di probabilità utilizzate nella modellazione statistica.

In particolare:

* definiremo in modo rigoroso le variabili casuali, discrete e continue, insieme alle loro distribuzioni di probabilità;
* introdurremo il *valore atteso*, strumento essenziale per comprendere il comportamento medio di una variabile casuale;
* discuteremo la *varianza* e la *deviazione standard*, che quantificano l’incertezza attorno al valore atteso;
* vedremo come queste idee costituiscano la base dell’inferenza statistica, sia frequentista che bayesiana.

Questo capitolo fornisce quindi il linguaggio tecnico indispensabile per tutto ciò che segue: distribuzioni a priori, modelli probabilistici, simulazioni predittive e, più avanti, la costruzione formale dell'inferenza bayesiana.
...
Nel linguaggio del calcolo delle probabilità, una *variabile casuale* rappresenta un ponte formale tra il mondo degli eventi qualitativi e quello dell'analisi numerica. Essa formalizza l'idea di assegnare un valore numerico a ciascun possibile risultato di un esperimento casuale, trasformando così esiti anche non numerici — come il seme di una carta da gioco o l'esito di un test medico — in quantità misurabili. Questa traduzione in valori numerici è fondamentale per applicare gli strumenti propri della teoria della probabilità e dell'analisi statistica allo studio dei fenomeni aleatori.

In termini formali, sia $\Omega$ uno spazio campionario, ovvero l'insieme di tutti i possibili risultati di un esperimento. Una variabile casuale $X$ è una funzione  

$$
X : \Omega \longrightarrow \mathbb{R}
$$
che assegna a ogni elemento $\omega \in \Omega$ un unico numero reale $X(\omega)$. 

Ad esempio, nel lancio di una moneta, dove $\Omega = \{T, C\}$, potremmo definire una variabile casuale $X$ tale che $X(T) = 1$ e $X(C) = 0$. In questo modo, un esito qualitativo viene codificato numericamente, permettendo di calcolare proprietà come il valore atteso o la varianza, e facilitando operazioni altrimenti complesse o impossibili su oggetti non numerici.

Grazie a questa definizione, è possibile modellare matematicamente il comportamento aleatorio e analizzare attraverso un quadro analitico coerente fenomeni probabilistici anche molto diversi tra loro.
...
![La variabile casuale $X$ rappresenta la somma di due dadi [figura tratta da @kroese2025statistical].](../../figures/random_variable_definition.png){ width=70% }
...
Le variabili casuali si dividono in due categorie principali: le variabili casuali discrete e le variabili casuali continue.
